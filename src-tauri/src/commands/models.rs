use crate::{
    db::DbState,
    types::{CreateModelInput, Model, UpdateModelInput},
};
...
pub(crate) fn list_models_for_asset(
    ...
    let mut stmt = conn.prepare(
        "SELECT m.id, m.name, m.display_name, m.created_at
         FROM models m
         INNER JOIN asset_models am ON am.model_id = m.id
         WHERE am.asset_id = ?
         ORDER BY COALESCE(m.display_name, m.name) COLLATE NOCASE",
    )?;
...
pub fn create_model(input: CreateModelInput, db: State<'_, DbState>) -> CommandResult<Model> {
    ...
    let conn = connection(&db)?;
    conn.execute(
        "INSERT INTO models (name, display_name) VALUES (?, ?)",
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, display_name, created_at FROM models WHERE id = ?")
        .map_err(db_error)?;

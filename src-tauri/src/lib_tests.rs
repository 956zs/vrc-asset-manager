...
fn assert_seeded_assets(db: &db::DbState) {
    let initial_assets = commands::assets::get_assets(AssetFilters::default(), command_state(db))
...
fn assert_exported_library_settings(export_path: &Path, expected_root: &Path) {
    ...
    let smoke_asset = exported_assets
        .iter()
        .find(|asset| {
            asset.get("displayName").and_then(|value| value.as_str()) == Some("Smoke Asset Updated")
        })
...
fn export_smoke_save(db: &db::DbState, demo_root: &Path) -> PathBuf {
    ...
    let export_summary =
        commands::saves::export_save(export_path.to_string_lossy().to_string(), command_state(db))
            .expect("export save");
...
fn import_smoke_save(db: &db::DbState, export_path: &Path, relations: SmokeRelations) {
    let import_summary =
        commands::saves::import_save(export_path.to_string_lossy().to_string(), command_state(db))
            .expect("import save");
...
fn managed_import_batch_preserves_optional_metadata() {
    ...
    let imported_assets = commands::assets::get_assets(AssetFilters::default(), command_state(&db))
...
fn managed_import_batch_handles_rename_conflict_and_unsupported_sources() {
    ...
    assert_eq!(
        report.results[2].failure_stage,
        Some(commands::assets::ImportFailureStage::Preflight)
    );
    assert!(report.results[2].final_path.is_none());
    assert!(report.results[2].message.contains(".7z / .rar"));
    assert!(report.results[2].message.contains("未來預定支援"));
...
fn managed_import_batch_reports_db_failure_after_file_operation() {
    ...
    assert_eq!(
        ...
        Some(commands::assets::ImportFailureStage::DbRecord)
    );
...
fn assert_vcc_snapshot(db: &db::DbState) {
    ...
    let snapshots = commands::vcc::scan_vcc_projects(command_state(db)).expect("scan vcc projects");
...
fn asset_health_reports_path_states() {
    ...
    let health =
        commands::assets::scan_asset_health(command_state(&db)).expect("scan asset health states");
